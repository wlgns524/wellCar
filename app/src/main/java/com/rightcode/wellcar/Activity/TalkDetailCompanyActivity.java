...
public class TalkDetailCompanyActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_talk_detail_company);
    ...
    @OnClick({R.id.tv_review_request, R.id.iv_talk_send, R.id.iv_bottom})
    void onClickMenu(View view) {
        switch (view.getId()) {
            case R.id.tv_review_request: {
                chatRegisterReview();
                break;
            }
            case R.id.iv_talk_send: {
                if (TextUtils.isEmpty(et_talk_send.getText())) {
                    ToastUtil.show(TalkDetailCompanyActivity.this, "내용을 입력해주세요");
                    break;
                }
    ...
    private void chatRoomDetail(Boolean isScrollBottom) {
        ChatRoomDetailRequester chatRoomDetailRequester = new ChatRoomDetailRequester(TalkDetailCompanyActivity.this);
    ...
    private void chatRegister(String content) {
        showLoading();
        ChatRegisterRequester chatRegisterRequester = new ChatRegisterRequester(TalkDetailCompanyActivity.this);
    ...
    private void chatRoomRemoveDialog() {
        CommonDialog commonDialog = new CommonDialog(TalkDetailCompanyActivity.this);
    ...
    private void chatRoomRemove() {
        showLoading();
        ChatRoomRemoveRequester chatRoomRemoveRequester = new ChatRoomRemoveRequester(TalkDetailCompanyActivity.this);
        ...
        request(chatRoomRemoveRequester,
                success -> {
                    CommonResult result = (CommonResult) success;
                    if (result.getCode() == 200) {
                        ToastUtil.show(TalkDetailCompanyActivity.this, "채팅방이 삭제되었습니다");
                        finishWithAnim();
                    } else {
                        showServerErrorDialog(result.getResultMsg());
                    }
    ...
    private void chatRegisterReview() {
        showLoading();
        ChatRegisterRequester chatRegisterRequester = new ChatRegisterRequester(TalkDetailCompanyActivity.this);

        ChatRegister param = new ChatRegister();
        param.setContent("review");
        param.setDiff(DataEnums.TalkDiffType.REVIEW);
